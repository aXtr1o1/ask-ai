...
async def test_get_sessions_for_user():
    ...
    mock_cursor.fetchall.return_value = [
        ("sess-001", "Asset Query", now, now, False, False, None, False,False),
        ("sess-002", "PPM Check",   now, now, True,  False, "Work", False, False),
    ]
    mock_cursor.description = [
        ("session_id",), ("title",), ("created_at",), ("updated_at",),
        ("is_pinned",), ("is_archived",), ("group_name",), ("is_space_booking",), ("is_advance",)
    ]
    mock_conn = MagicMock()
