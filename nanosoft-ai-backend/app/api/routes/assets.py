...
ASSETS_SP_AGGREGATE_GROUP_COLUMNS = set(TOOL_GROUP_BY_COLUMNS.get("ASSETS", []))

def _assets_requires_local_aggregate(req: AssetRequest) -> bool:
    group_cols = set(req.group_by_columns or [])
    if group_cols - ASSETS_SP_AGGREGATE_GROUP_COLUMNS:
        return True
    return False

def _format_local_assets_aggregate(rows: list[dict], group_by_columns: list[str]) -> dict:
...
def _call_local_assets_aggregate(req: AssetRequest) -> dict:
    query_req = req.model_copy(update={
        "is_aggregate": False,
        "group_by_columns": None,
        "aggregate_function": None,
    })
    formatted = _call_sp_asset_query(query_req)
    rows = formatted.get("p_list") or []
    return _format_local_assets_aggregate(rows, req.group_by_columns or [])
...
def get_assets(req: AssetRequest):
    ...
    if getattr(req, "is_aggregate", False):
        ...
        if _assets_requires_local_aggregate(req):
            logger.info("📊 [GET-ASSETS] Local Aggregation Fallback triggered")
            return _call_local_assets_aggregate(req)
