...
def get_ppm(req: PPMRequest):
    ...
    try:
        ...
        # B) Try mapping the keyword to location/entity fields (spot_name, building, locality, equipment),
        ...
        if not p_list:
            ...
            if req.keyword:
                for field in ["spot_name", "building", "locality", "equipment"]:
            ...
            for field, original_val, is_keyword_mapping in fallback_items:
                ...
                for candidate in unique_candidates:
                    logger.info(
                        "🔄 Retrying PPM query mapping %s%s to candidate='%s'...",
                    ...
                    cursor.callproc("sp_ppm_query", [
                        ...
                        req.contract,
                        req.tech,
                        candidate if field == "equipment" else req.equipment,
                    ...
                    p_list = formatted.get("p_list", [])
                    if p_list:
                        logger.info("✅ PPM fallback retry succeeded with %s='%s'!", field, candidate)
