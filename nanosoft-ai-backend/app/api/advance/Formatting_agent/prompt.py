...
def build_formatting_prompt(
    ...
    internal_detail_guardrail = (
        ...
        "If Shape is 'error' or 'insufficient_data', translate the underlying cause into "
        "plain, confident language a Facility Management user would understand — never "
        "say \"the operation/step/tool failed\" or reference how the system is built internally.\n\n"

        "Distinguish these four situations precisely — they are not interchangeable:\n"
        ...
        "  2. Shape is 'insufficient_data' — the query ran correctly and reached the right "
        "data, but that data itself didn't have enough usable values to complete one specific "
        "computation in the plan. The Reason names what was being computed and why the data "
        "fell short — use it to identify which planned step that is, and state once, in plain "
        "language grounded in that reason, that it could not be completed because the available "
        "data isn't sufficient for it. This is a fact about the data, not a system error, so "
        "don't say anything failed or apologize for a malfunction. Then look at the rest of the "
        "plan: any later step whose own inputs are drawn from that step's output could not "
        "produce its requested result either, for that same underlying reason — say plainly, in "
        "terms of what the user actually asked for, which further results these were, without "
        "restating the reason itself again for each one. If nothing later in the plan depended "
        "on it, the one statement is enough.\n"
        "  3. Shape is 'error' — something else prevented reaching a result (a planning or "
        "execution issue). Clearly say the answer could not be determined right now, without "
        "implying the value is zero and without any internal jargon.\n"
        "  4. A normal, successfully computed non-zero result — state it directly. If the "
        "Reason mentions a data quality caveat (e.g. a field used to group or compute the "
        "answer was mostly blank in the matching records), still give the result, but add one "
        "brief, plain-language qualifier so the user knows to treat it as based on limited data — "
        "don't omit the caveat and don't overstate the result's precision."
    )
