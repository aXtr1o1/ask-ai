...
class AssetsInput(BaseModel):
    ...
    on_hold: Optional[bool] = Field(None, description="FILTER only: set true/false when user wants assets with a specific OnHold value (e.g. 'assets on hold'). Do NOT use when user asks 'how many OnHolds' or wants a true/false breakdown — use is_aggregate=True with group_by_columns=['OnHold'] instead.")
    ...
    enable_ppm: Optional[bool] = Field(None, description="FILTER only: set true/false for a specific EnablePPM value. For breakdown use is_aggregate=True with group_by_columns=['EnablePPM'].")
    enable_bdm: Optional[bool] = Field(None, description="FILTER only: set true/false for a specific EnableBDM value. For breakdown use is_aggregate=True with group_by_columns=['EnableBDM'].")
    ...
    is_aggregate: Optional[bool] = Field(default=False, description="Set True when user asks grouping/breakdown questions like 'how many per division', 'breakdown by building', OR 'how many OnHolds are there' (column value breakdown). For normal filter/list queries leave as False.")
    group_by_columns: Optional[List[str]] = Field(default=None, description="List of columns to group by. Only fill when is_aggregate=True. Example: ['DivisionName'], ['BuildingName', 'FloorName'], or ['OnHold'] for true/false counts. Valid columns: DivisionName, DisciplineName, BuildingName, FloorName, LocalityName, StatusName, ConditionName, PriorityName, AssetTypeName, EquipmentName, MakeName, ModelName, SpotName, TradeGroupName, ServiceAreaName, YearOfManuf, OnHold, IsSnagged, IsScraped, EnablePPM, EnableBDM.")
...
class FAInput(BaseModel):
    ...
    is_withdraw: Optional[bool] = Field(None, description="FILTER only for a specific IsWithdraw value. For breakdown use is_aggregate=True with group_by_columns=['IsWithdraw'].")
    is_rework: Optional[bool] = Field(None, description="FILTER only for a specific IsRework value. For breakdown use is_aggregate=True with group_by_columns=['IsRework'].")
    is_active: Optional[bool] = Field(None, description="FILTER only for a specific IsActive value. For breakdown use is_aggregate=True with group_by_columns=['IsActive'].")
...
class SBInput(BaseModel):
    ...
    tech: Optional[str] = Field(None, description="Technician name. Map if user mentions 'Technician' or 'Tech'.")
    is_withdraw: Optional[bool] = Field(None, description="FILTER only for a specific IsWithdraw value. For breakdown use is_aggregate=True with group_by_columns=['IsWithdraw'].")
    is_reschedule: Optional[bool] = Field(None, description="FILTER only for a specific IsReschedule value. For breakdown use is_aggregate=True with group_by_columns=['IsReschedule'].")
    is_rework: Optional[bool] = Field(None, description="FILTER only for a specific IsRework value. For breakdown use is_aggregate=True with group_by_columns=['IsRework'].")
    is_active: Optional[bool] = Field(None, description="FILTER only for a specific IsActive value. For breakdown use is_aggregate=True with group_by_columns=['IsActive'].")
    keyword: Optional[str] = Field(None, description="Fallback for specific technical terms not covered by other fields.")
    date_from: Optional[str] = Field(None, description="Start date range. Use YYYY-MM-DD.")
    ...
    group_by_columns: Optional[List[str]] = Field(default=None, description="Columns to group by. Valid: DivisionName, DisciplineName, BuildingName, FloorName, LocalityName, PPMStageName, FrequencyName, ServiceTypeName, ContractName, IsWithdraw, IsReschedule, IsRework, IsActive.")
    aggregate_function: Optional[str] = Field(default=None, description="COUNT for how many, SUM for total, AVG for average. Only when is_aggregate=True.")
