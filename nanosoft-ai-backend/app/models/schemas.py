...
class ContractInput(BaseModel):
    ...
    date_from: Optional[str] = Field(None, description="Filters contracts by their start date. Use this when the user asks about contracts that started from a certain point in time. When the user names a specific date, also set date_to to the same value to match exactly that day. Use YYYY-MM-DD format. Relative keywords like 'today', 'this month', 'last month', 'this year' are also supported.")
    date_to: Optional[str] = Field(None, description="The upper bound of the contract start date filter — not the contract end or expiry date. Set this alongside date_from to define a start date range, or set it equal to date_from when the user names a specific start date. Do not use this field for 'contracts ending in' queries — those cannot be filtered this way. Use YYYY-MM-DD format. Relative keywords are supported.")
    ...
    is_aggregate: Optional[bool] = Field(default=False, description="Set True only when the user wants a grouped count distributed across a contract-level categorical dimension. For all other cases — including simple totals, filtered counts, and questions about how many contracts have a particular attribute or characteristic — leave this False and use the appropriate filter fields or keyword instead.")
    group_by_columns: Optional[List[AllColumns]] = Field(default=None, description="The database columns to group the result by when is_aggregate=True. ONLY use category-level dimension columns — NEVER use per-row identity fields like ContractName, ContractIDPK, or ContractCode. Valid values: ContractTypeName, ContractCategName, ContractGroupName, OrganisationName, CustomerName, ContStStatus, ContStTypes, IsActive, IsDraft, IsRenewal, IsExtended, IsTerminate, IsPPM, IsBDM, IsDSM, IsIncident, IsCase, IsNonContract, Period, TaxName, ConPaymentTermsName.")
