...
- COLUMN VALUE BREAKDOWN (CRITICAL): If user asks "how many [ColumnName]?" or "how many [column] are there?" about a data column (especially boolean columns like OnHold, IsSnagged, StatusName, PriorityName), treat it as an aggregate query — set is_aggregate=True, group_by_columns=[exact column name], aggregate_function="COUNT". Do NOT set the corresponding filter parameter. Return a count per distinct value (e.g., OnHold true: 15, OnHold false: 230). Only use a filter when the user specifies a single value (e.g., "how many assets on hold" → on_hold=true filter, NOT aggregate).
